        ...
        public static HttpResponse GetResponse(HttpRequest request)
        {
            ...
            try
            {
                using (var httpClient = GetHttpClient(request))
                {
                    ...
                    var task = request.Method.Value == MethodType.GET ? httpClient.GetAsync(httpRequestMessage.RequestUri) : httpClient.PostAsync(httpRequestMessage.RequestUri, null);
                    //task.Wait();
